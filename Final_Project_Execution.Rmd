...
for (i in 1:num_pages) {
  current_url<-paste("http://raw.githubusercontent.com/pm0kjp/IS607_Final_Project/master/", remote_directory_name, "/Page-", i, ".html", sep="")
  filename<-paste(local_directory_name, "/Page-", i, ".html", sep="")
  download.file(current_url, filename, method="curl")
}
...
# It probably makes sense for us to come up with a huge data frame that includes all the rows of the above
# data frames with one more column stating the serious / satire category.
...
# Now we can do data vis like
boxplot(all_reviews$num_excl_review~all_reviews$category)
boxplot(all_reviews$num_offensive_text~all_reviews$category)  # Something here!
...
# I create a function that will take any of my data frames and create text files, which will contribute
# to each corpus.
...
satire_corpus<-createCleanCorpus("amazon_satire/corpus", "hutzler banana slice slicer bic cristal pen for her denon akdl dedicated link cable discontinued by manufacturer samsung uns framed inch k ultra hd d smart led tv uranium ore")

satire_tdm <- TermDocumentMatrix(satire_corpus)   


serious_corpus<-createCleanCorpus("amazon_serious/corpus", "oxo apple grip core corer divide divider slice slicer bic pen velocity mediabridge hdmi black ultra cable feet madagascar gemstone gemstones stone stones labradorite septarian chrysocolla samsung inch p unj smart tv")
...
Let's take a look at what we have.  We'll take a look at the first and last 5 columns.

```{r}
head(df_for_model[, c(1:5, 9963:9967)])
```
...
Note that we're dealing with 2000 cases, so predict() will churn for a while before completing.
...
# Not sure why everything is showing up as "serious"...
...
# Well, it's better than Bayes for sure, but not fantastic.  68.5% accuracy on satire vs. serious classification.

# What if we tried to discern the important words and came up with a different nb model?

# First, find the terms that appear in each corpus > 30X
...
#Not sure what you wanted to plot here, Karen -- strength of predictors?

#nb.res <- NaiveBayes(review_category ~ ., data=df_for_model[df_train,])
...
#nb.pred <- predict(nb.res, df_for_model[-df_train,])

# raw accuracy
#confusion.mat <- table(nb.pred$class, df_for_model[-df_train,"satire"])
