...
# normal_tv_review<-getReviews("http://www.amazon.com/Samsung-UN40J5200-40-Inch-1080p-Smart/product-reviews/B00WR292JE/?sortBy=byRankDescending&pageNumber=", corpus_identifier="amazon_serious")
# above has bad html due to Amazon robot check .. will rerun and add back to repo.
```
...
# normal_tv_reviews<-countWords(normal_tv_reviews)  this has partial HTML thanks to Amazon robot check -- will add back.
```
...
makeCorpus("amazon_satire/corpus", bic_reviews)
makeCorpus("amazon_satire/corpus", banana_slicer_reviews)
makeCorpus("amazon_satire/corpus", cable_reviews)
makeCorpus("amazon_satire/corpus", tv_reviews)
makeCorpus("amazon_satire/corpus", uranium_reviews)
...
satire_corpus<-preprocessCorpus("amazon_satire/corpus", "hutzler banana slice bic cristal for her denon akdl dedicated link cable discontinued by manufacturer samsung uns framed inch k ultra hd d smart led tv uranium ore")

dtm <- DocumentTermMatrix(satire_corpus)   
inspect(dtm)
```
...
df_for_model <- as.data.frame(dtm)
...
# apply NB classifier
nb.res <- NaiveBayes(satire ~ ., data=df_for_model[df_train,])
